...
export default class AuthUtil {
  public static getAuthProviderUserInfo = (
  ...
  public static isLinkedAuthProvider = (
    authenticatedUser: UserInfo,
    authProvider: AuthProvider
  ): boolean => {
    let flag = false;
    if (authenticatedUser && authenticatedUser.providerData) {
      ...
      flag = linkedProviderIds.some(
        linkedProviderId => toAuthProvider(linkedProviderId) === authProvider
      );
    } else {
      flag = false;
    }
